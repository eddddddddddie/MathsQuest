...
- **index.html** (~215 lines) — All screens (title, character select, world select, gameplay with HUD/item bar/challenge modal/level intro, world complete, game over, victory), scanline overlay
- **styles.css** (~340 lines) — Retro pixel theme via CSS variables, Press Start 2P font, 3D pixel-border buttons, scanline CRT effect, arcade-style character select layout
- **game.js** (~1520 lines) — Entire game engine
...
4. **CharacterDesigner** (~131-225) — 8-bit pixel sprite preview on 800x1040 canvas (10px pixel blocks). `render(view)` accepts view param (0=front, 1=side, 2=back) but currently always called with 0. `renderMini(canvasId)` stamps cropped version into small canvases. `setOption(k,v)` updates options dict and re-renders.
5. **drawPixelItem()** (~207-225) — Standalone function drawing 12 sprite types (key, gem, potion, scroll, orb, cup, ring, feather, compass, skull, music) as 16x16 pixel art using `fillRect`.
6. **WORLDS[]** (~228-830) — Array of 10 world configs. Each has:
...
7. **PlatformerEngine** (~833-1200) — Canvas game loop:
...
8. **App** (~1206-end) — Top-level controller:
...
- GitHub Pages compatibility: `sound.init()` wrapped in try-catch, uses `window['webkitAudioContext']` not `window.webkitAudioContext`. `init()` ensures `bindEvents()` runs even if particles/canvas fail. Splash screens that block interaction before user gesture break on GH Pages — the current approach puts "LEWIS GAMES INC." as text on the title screen instead.
...
Enchanted Forest, Underwater Ruins, Haunted Mansion, Desert Temple, Artemis II (space), Down on the Farm, Mushroom Kingdom (Mario), Trump Tower, Freo Dockers (AFL), Leighton Beach. Each has unique: background with animated elements, music track (recognizable melodies — Old MacDonald, Space Oddity, Star-Spangled Banner, Surfin' USA, Freo club song, etc.), player costume, and themed items/enemies.
